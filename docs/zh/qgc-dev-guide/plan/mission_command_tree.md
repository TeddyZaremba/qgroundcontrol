...
- 根-通用Mavlink
  ...
  - 固件类型特定——每种固件类型的一个可选叶节点 (PX4/ArduPilot)
    - 特定载具型号——每种载具类型有一个可选的叶节点(FW/MR/VTOL/Rover/Sub)
